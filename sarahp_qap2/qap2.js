...
function snake(value) {
    ...
    value = value.toLowerCase().trim().replace(/[\s\t\.]+/g, "_");
      // this is a combination (in order) of:
  ...
  function createVideo(src="", width=500, controls=false) {
  ...
  function validDateString(value) {
    ...
    if (!re.test(value)){
        throw new Error(`Invalid date string: '${value}'. Expected format is 'YYYY-MM-DD'`)
    }

    const groups = value.match(re).groups;  
    let year = groups.year;
    let month = groups.month;
    let day = groups.day;

    try {
        const result = validYear(year);
      } catch (error) {
        console.error(error.message); 
      }

    try {
        const result = validMonth(month);
    } catch (error) {
        console.error(error.message); 
    }
    
    try {
        const result = validDay(day, month);
        } 
    catch (error) {
        console.error(error.message); 
    }
  ...
  function validYear(str){
    const re = /^\d{4}$/;
    if (!re.test(str)){
        throw new Error(`Invalid year: '${str}'. Expected format is 'YYYY'`)
    }
    console.log("Valid year string:", str); // This will log the valid string
    return str;
  }

  function validMonth(str){
    const re = /^\d{2}$/;
    if (re.test(str) && Number(str) <= 12){
        console.log("Valid month string:", str); // This will log the valid string
        return str;
    }else
        {throw new Error(`Invalid month: '${str}'. Expected format is 'MM'`)
    }
  }



  function validDay(str, month){
    const re = /^\d{2}$/;
    let day = Number(str);
    if (!re.test(str)){
        throw new Error(`Invalid day: '${str}'. Expected format is 'DD'`)
    }
    if (month == '02' && day > 28){
        throw new Error(`Invalid Month & Day: '${str}'. Expected format is 'DD'. (Note, if it is Feb 29th, please write the day as Feb 28th because our coders haven't learned how to handle leap years.`)
    }
    if (month in ['04', '06', '09', '11'] && day > 30){
        throw new Error(`Invalid Month & Day: '${str}'. Please confirm the date.`)
    }
    if (day > 31){
        throw new Error(`Invalid Day: '${str}'.`) 
    }
    console.log("Valid day string:", str); // This will log the valid string
    return str;
  }
  ...
  function parseDateString(value) {
    try {
        const result = validDateString(value);
      } catch (error) {
        console.error(error.message); // Logs: "Invalid date string: 'invalid-input'. Expected format is 'YYYY-MM-DD'.
        return value;
      }
  ...
  // This will create a datestring Month, DD, YYYY 
  function toLongDateString(dateObject){
    date = dateObject
  ...
  function validCoords(value) {
    ...
    let re = /^\[?(?<group1>-?\d*.\d*),\s*(?<group2>-?\d*.\d*)\]?$/;
    let test = true;
    ...
    // validate latitude
    try {
        const result = validLong(long);
    } catch (error) {
        console.error(error.message);
        test = false;  
    }

    return [test, lat, long];
  }


  function validLat(value){
    let coords = parseFloat(value)
    if (-90 <= coords && coords <= 90){
      console.log("Valid Latitude:", value); // 
      return value;
    }
    else{
  ...
  function validLong(value){
    let coords = parseFloat(value)
    if (-180 <= coords && coords <= 180){
      console.log("Valid Longitude:", value);
      return value;
    }
    else{
  ...
  function normalizeCoord(value) {
    ...
    try {
      const [ test, lat, long ] = validCoords(value);
      if (test == true){
        let coords = `(${lat}, ${long})`;
        return coords}
      else{
  ...
  function formatCoords(...values) {
    ...
    array.forEach(item => { 
      [test, lat, long] = validCoords(item);
      if (test == true){coordArray.push(`(${lat}, ${long})`)}
    });
    return coordArray
  }
  ...
  //Solution, Option 1a (dictionary instead of switch): 

  const mimeValues = [
    { extension: /^htm(l)?$/, mime: "text/html" }, // Matches "html" or "htm"
    { extension: /^jp(e)?g$/, mime: "image/jpeg" }, // Matches "jpg" or "jpeg"
    { extension: /^(ico|cur)$/, mime: "image/x-icon" }, // Matches "ico" or "cur"
  ...
  function mimeFromFilename2(filename) {
    let ext = getExtension(filename);
    console.log(ext);
  
    for (let item of mimeValues) {
      if (item.extension.test(ext)) {
        return item.mime; // Directly return the mime when a match is found
      }}
    console.log(ext);
    return "application/octet-stream"; // Return default if no match is found
    }
  
  // solution option 2 (with Switch): 
  
  function mimeFromFilename(filename) {
    // NOTE: Use a switch statement in your solution.
  
    let ext = filename.trim().replace(/.*\./, "")
    ...
    switch (ext) {
        ...
        case "cur":
          return "image/x-icon";
        default:
          return "application/octet-stream"; // Default if no match is found
      }
  }
  ...
  function getlicenseURL(licenseCode){
    ...
    const code = result ? licenseCode.replace("CC-", "").toLowerCase() : "default";
  ...
  function getDescriptionByLicense(licenseKey) {
    ...
    return result ? result.descr : "All Rights Reserved"; // Return description in dictionary or default description if not found
  }
  ...
  function validBoolValue(value){
    if (typeof value == "boolean"){
      return value
    }
    if (typeof value == "string"){
      try{
      strValue = toBoolStr(value);
  ...
  function toBoolStr(str){

    let value = str.trim().toUpperCase()
    let c = value[0];

    if (['Y', 'O', 'T', 'V'].includes(c)){
      return true}
    if (['N', 'F'].includes(c)){
      return false}
    else {
  ...
  function every(...values) {
    ...
    array = [...values]
    array.forEach(item => {
      let value = validBoolValue(item);
      if (value == false){test = false}}
    )
    console.log(test);
    return test
  }
  ...
  function any(...values) {
    ...
    let array = [...values]
    array.forEach(item => {
      if (validBoolValue(item) == true){test = true}}
    )
    console.log(test);
    return test
  }
  ...
  function buildURL(query=String, count=25, order=true, license='any'){

    let apiURL = "https://api.inaturalist.org/v2/observations"
  
    query = buildQuery(query);
    count = buildCount(count);
    order = buildOrder(order);
    license = buildLicense(license);
  
    let url = `${apiURL}${query}&${count}&${order}&${license}`;
    return url
  }
  ...
  function buildCount(count=Number){
    if (1 <= count && count <=50){return `count=${count}`}
    else{
      console.error(`Invalid value: '${count}'. Count should be between 1-50. Default count of 25 returned`);
      return `count=25`
    }
  }
  
  function buildOrder(order=Boolean){
    if (order){return `order=ascending`}
    if (!order){return `order=descending`}
    else{
      console.error(`Invalid value: '${order}'. Order should be "true" (=ascending) or "false" (=descending). Default value "true" returned.`);
      return `order=ascending`
    }
  }
  
  function buildLicense(license=String){
    licenseList = ['none', 'any', 'cc-by', 'cc-by-nc', 'cc-by-sa', 'cc-by-nd', 'cc-by-nc-sa', 'cc-by-nc-nd'];
    if (licenseList.includes(license)){return `license=${license}`}
    else{
      console.error(`Invalid value: '${license}'. Default license of 'any' returned.`);
      return `license=any`
    }
  }